...
export default function Dashboard() {
    ...
    try {
      const [eq, ot, pn, pc, pl] = await fetchWithRetry(() =>
          Promise.all([
            ...
            pb.collection("plans_preventifs").getFullList({ filter: "statut = 'actif'" }),
          ])
      );
      setData({ equipements: eq, ordresdetravail: ot, pannes: pn, pieces: pc, plans_preventifs: pl });
    } catch (e) {
      console.error(e);
      setError("Impossible de charger les données. Vérifiez votre connexion PocketBase.");
    } finally {
      setLoading(false);
      setRefreshing(false);
    }
  ...
  const metricspreventif = useMemo(() => {
    const { plans_preventifs } = data;
    const now = new Date();
    now.setHours(0, 0, 0, 0);

    // Calcule la prochaine date selon la fréquence
    const getProchainDate = (plan) => {
      if (!plan.date_debut) return null;
      ...
      if (!freqDays) return debut; // fréquence inconnue → date de début

      // Avancer jusqu'à la prochaine occurrence >= aujourd'hui
      const prochaine = new Date(debut);
      while (prochaine < now) {
        prochaine.setDate(prochaine.getDate() + freqDays);
      }
      return prochaine;
    };

    const upcomingInterventions = plans_preventifs
        .map(plan => ({ ...plan, _prochaine: getProchainDate(plan) }))
        .filter(plan => {
          if (!plan._prochaine) return false;
          const diffDays = Math.round((plan._prochaine - now) / (1000 * 60 * 60 * 24));
          return diffDays >= 0 && diffDays <= 7;
        })
        .sort((a, b) => a._prochaine - b._prochaine);

    return {
      ...metrics,
      interventionsProchaines: upcomingInterventions.length,
      prochainRappel: upcomingInterventions[0] || null,
      // prochainRappel._prochaine = la date calculée à afficher
    };
  }, [data, metrics]);
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
                ...
                <div className="bg-white rounded-2xl p-5 border border-gray-300 shadow-lg">
                  ...
                  <div className="text-sm text-gray-500 mt-1 flex items-center gap-2">
                    ...
                    {metrics.equipementsHS > 0 &&
                        <span className="text-red-600 font-medium">• {metrics.equipementsHS} HS</span>}
                  </div>
                </div>
                ...
                <div className="bg-white rounded-2xl p-5 border border-gray-300 shadow-lg">
                  ...
                  <div className="text-sm text-gray-500 mt-1 flex items-center gap-2">
                    ...
                    {metrics.otPrioriteHaute > 0 &&
                        <span className="text-red-600 font-medium">• {metrics.otPrioriteHaute} critiques</span>}
                  </div>
                </div>
                ...
                <div className="bg-white rounded-2xl p-5 border border-gray-300 shadow-lg">
                  ...
                  <div className="text-2xl font-bold text-slate-800">{metricspreventif.plansActifs}</div>
                  <div className="text-sm text-gray-500 mt-1">
                    {metricspreventif.interventionsProchaines > 0 ? (
                        <div className="space-y-1">
                          <div className="flex items-center gap-1 text-amber-600 font-medium">
                            <Clock className="w-3 h-3" />
                            <span>{metricspreventif.interventionsProchaines} intervention{metricspreventif.interventionsProchaines > 1 ? "s" : ""} cette semaine</span>
                          </div>
                          {metricspreventif.prochainRappel && (
                              <div className="text-xs text-gray-500 truncate">
                                Prochaine: {metricspreventif.prochainRappel.nom || "Maintenance"} - {formatDate(metricspreventif.prochainRappel._prochaine)}
                              </div>
                          )}
                        </div>
                    ) : (
                        <div className="text-green-600 font-medium">Aucun rappel imminent</div>
                    )}
                  </div>
                </div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div className="bg-white rounded-2xl border border-gray-300 shadow-lg overflow-hidden">
                  ...
                  <div className="divide-y divide-gray-50 max-h-[320px] overflow-y-auto">
                    {data.ordresdetravail.slice(0, 5).map(ot => (
                ...
                <div className="bg-white rounded-2xl border border-gray-300 shadow-lg overflow-hidden">
                  ...
                  <div className="divide-y divide-gray-50 max-h-[320px] overflow-y-auto">
                    {data.pannes.slice(0, 5).map(panne => (
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mt-6">
                ...
                <div className="bg-white rounded-2xl border border-gray-300 shadow-lg p-5">
                  ...
                  <div className="space-y-3">
                    {[
                      { label: "Opérationnel", count: metrics.equipementsOp, color: "bg-green-500" },
                      { label: "Maintenance", count: countBy(data.equipements, "statut", "maintenance"), color: "bg-orange-500" },
                      { label: "Hors service", count: metrics.equipementsHS, color: "bg-red-500" },
                    ].map(item => (
                        <div key={item.label}>
                          ...
                          <div className="w-full bg-gray-100 rounded-full h-2">
                            <div
                                ...
                                style={{ width: `${metrics.equipementsTotal > 0 ? (item.count / metrics.equipementsTotal) * 100 : 0}%` }}
                            />
                          </div>
                        </div>
                    ))}
                ...
                <div className="bg-gradient-to-br from-indigo-600 to-blue-700 rounded-2xl shadow-lg p-5 text-white flex flex-col justify-between">
                  <div>
                    ...
                    <p className="text-blue-100 text-sm leading-relaxed">
                      ...
                      {metrics.pannesNouvelles > 0
                          ? ` ${metrics.pannesNouvelles} nouvelle${metrics.pannesNouvelles > 1 ? "s" : ""} panne${metrics.pannesNouvelles > 1 ? "s" : ""} à traiter.`
                          : " Aucune nouvelle panne signalée."}
                    </p>
                  </div>
