...
// ✨ NOUVEAU : PlusIcon pour le bouton d'ajout
const PlusIcon = () => (
      ...
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-lg p-6 mx-4 max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            <input type="text" placeholder="ex: Contrôle mensuel pompe" value={form.nom} onChange={set("nom")}
              className="w-full border border-gray-600 rounded-xl px-4 py-2.5 text-sm outline-none focus:ring-2 focus:ring-indigo-400 text-black" required />
          </div>
          ...
          <div>
            ...
            <select value={form.equipement} onChange={set("equipement")} required
              ...
              {equipements.map((eq) => <option key={eq.id} value={eq.id}>{eq.nom} {eq.zone ? `— ${eq.zone}` : ""}</option>)}
            </select>
          </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <select value={form.frequence} onChange={set("frequence")}
                className="w-full border border-gray-600 rounded-xl text-black px-4 py-2.5 text-sm outline-none focus:ring-2 focus:ring-indigo-400 bg-white">
                {Object.entries(FREQUENCE_CONFIG).map(([key, { label }]) => <option key={key} value={key}>{label}</option>)}
              </select>
            </div>
            <div>
              ...
              <select value={form.statut} onChange={set("statut")}
                className="w-full border border-gray-600 rounded-xl text-black px-4 py-2.5 text-sm outline-none focus:ring-2 focus:ring-indigo-400 bg-white">
                {Object.entries(STATUT_CONFIG).map(([key, { label }]) => <option key={key} value={key}>{label}</option>)}
              </select>
            </div>
          </div>
          ...
          <div className="flex justify-end gap-3 pt-4 border-t">
            <button type="button" onClick={onClose} className="px-4 py-2 rounded-xl text-sm font-medium text-gray-600 hover:bg-gray-100">
              Annuler
            </button>
...
export default function CalendrierPreventif() {
  ...
  // ✨ NOUVEAU : State pour le modal d'ajout
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        unsub = await pb.collection("plans_preventifs").subscribe("*", () => fetchPlans({ silent: true }));
      } catch (_) {}
    })();
  ...
  filteredPlans.forEach(plan => {
    const occs = generateOccurrencesForMonth(plan, currentYear, currentMonth);
    occs.forEach(occ => {
  ...
  // ✨ NOUVEAU : Handler pour ouvrir le modal d'ajout
    ...
    <div className="p-4 bg-gray-50 min-h-screen rounded-xl font-sans">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 mb-4">
        <div className="flex items-center gap-3">
          <div>
            ...
            <p className="text-sm text-gray-500">{totalInterventions} interventions ce mois • {upcomingCount} à venir</p>
          </div>
        </div>

        <div className="flex items-center gap-2">
          {/* ✨ NOUVEAU : Bouton "+" pour ajouter une intervention */}
          <button
          ...
          <button
            ...
            className={`flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-medium transition-colors ${showFilters ? "bg-indigo-100 text-indigo-700" : "bg-white text-gray-600 border border-gray-200 hover:bg-gray-50"}`}
          >
            <FilterIcon /> Filtres
          </button>
          <button
      ...
      {showFilters && (
        <div className="bg-white rounded-2xl border border-gray-200 shadow-lg p-4 mb-4 flex flex-wrap gap-3 items-end">
          ...
          <button onClick={() => { setFilterEquipement(""); setFilterStatut(""); }}
      ...
      <div className="bg-white rounded-2xl border border-gray-300 overflow-hidden shadow-lg">
        ...
        <div className="flex items-center justify-between px-5 py-3 border-b border-gray-100 bg-gray-50/50">
          ...
          <div className="flex items-center gap-3">
            <h2 className="text-lg font-bold text-slate-800 min-w-[200px] text-center">{MOIS[currentMonth]} {currentYear}</h2>
            <button onClick={goToToday} className="text-xs font-semibold text-indigo-600 hover:text-indigo-700 bg-indigo-50 hover:bg-indigo-100 px-3 py-1.5 rounded-lg transition-colors">
              Aujourd'hui
            </button>
          </div>
            ...
            calendarCells.map((cell, idx) => {
              ...
              return (
                <div
                  ...
                  `}
                  onClick={() => {
                    if (occs.length > 0) setSelectedOccurrence(occs);
                  }}
                >
                  ...
                  <div className="flex items-center justify-between mb-1">
                    ...
                    {/* ✨ NOUVEAU : Petit "+" qui apparaît au survol pour ajouter rapidement */}
                    {isCurrentMonth && (
                      <button
      ...
      {selectedOccurrence && selectedOccurrence.length > 0 && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm" onClick={() => setSelectedOccurrence(null)}>
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-lg p-6 mx-4 max-h-[80vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-lg font-semibold text-slate-800">
                {selectedOccurrence.length === 1 ? "Détail de l'intervention" : `${selectedOccurrence.length} interventions`}
              </h3>
              <button onClick={() => setSelectedOccurrence(null)} className="w-8 h-8 flex items-center justify-center rounded-lg hover:bg-gray-100 text-gray-500">
                <XIcon />
              </button>
            </div>
                  ...
                  <div key={occ.id} className="border border-gray-200 rounded-xl p-4">
                    ...
                    <div className="grid grid-cols-2 gap-3 text-sm">
                      <div className="flex items-center gap-2 text-gray-600">
                        <CalendarIcon />
                        <span>{occ.date.toLocaleDateString("fr-FR", { weekday: "long", day: "numeric", month: "long", year: "numeric" })}</span>
                      </div>
                      <div className="flex items-center gap-2 text-gray-600">
                        <ClockIcon />
